...
class KiCADInterface(BoardPersistenceMixin):
        ...
        try:
            ...
            if handler:
                ...
                return {
                    ...
                    "hint": "This tool name is not registered. Check the spelling against the tool list.",
                }
    ...
    # The S-expression text helpers below are thin delegations to
    # utils.sexpr (pure, dependency-free, unit-tested there). They remain on
    # the class so existing callers — handlers/schematic_component.py via
    # ``iface._find_matching_paren`` and the ``cls._escape_sexpr_string``
    # tests — keep working unchanged.
    @staticmethod
    def _escape_sexpr_string(value: str) -> str:
        """Escape a string for safe insertion into an S-expression double-quoted token."""
        return sexpr.escape_sexpr_string(value)

    @staticmethod
    def _find_matching_paren(s: str, start: int) -> int:
        """Return the index of the closing paren matching the opening paren at `start`."""
        return sexpr.find_matching_paren(s, start)
    ...
    def _remove_property_from_block(self, block: str, name: str) -> Tuple[str, bool]:
        ...
        return sexpr.remove_property_from_block(block, name)
